...
const HighlightCard = ({ icon, badge, badgeColor, title, value, sub, accent }) => (
  <div className={`bg-white border ${accent || 'border-blue-100'} px-6 py-6 flex flex-col gap-3 relative overflow-hidden`}>
    ...
    <div className={`absolute bottom-0 left-0 right-0 h-0.5 ${accent?.replace('border-', 'bg-') || 'bg-blue-200'}`} />
  </div>
)
...
const SalesReport = () => {
  ...
  const highlights = useMemo(() => {
    ...
    const bestPeriodLabel = bestPeriod && bestPeriod.id ? (() => {
      const id = bestPeriod.id
      ...
      if (id.y != null && id.m != null) return `${id.y}-${String(id.m).padStart(2,'0')}`
      return '—'
    })() : null
  ...
  const handleDownloadPDF = async () => {
    ...
    setPdfLoading(true)
    try {
      const BLUE = [37, 99, 235]; const BLIGHT = [219, 234, 254]
      ...
      const addPageDecor = () => {
        ...
        doc.setFillColor(...BLIGHT); doc.rect(0, 287, W, 10, 'F')
      ...
      doc.setDrawColor(...BLUE); doc.setLineWidth(0.5); doc.line(14, 46, W - 14, 46)
      let y = 52

      const sectionHeader = (title) => {
        if (y > 255) { doc.addPage(); addPageDecor(); y = 28 }
        doc.setFillColor(...BLIGHT); doc.roundedRect(14, y, W - 28, 8, 1, 1, 'F')
      ...
      if (totals) {
        ...
        cards.forEach(([lbl, val], i) => {
          ...
          doc.setFillColor(...WHITE); doc.setDrawColor(...BLUE); doc.setLineWidth(0.3)
      ...
      if (highlights) {
        ...
        const hlRows = []
        if (highlights.bestBranch)  hlRows.push([' Best Branch',    highlights.bestBranch.branchData?.name  || highlights.bestBranch.id,  peso(highlights.bestBranch.finalAmount)])
        if (highlights.worstBranch) hlRows.push([' Needs Attention', highlights.worstBranch.branchData?.name || highlights.worstBranch.id, peso(highlights.worstBranch.finalAmount)])
        if (highlights.bestService) hlRows.push([' Top Service',     highlights.bestService.name,                                           peso(highlights.bestService.revenue)])
        if (highlights.bestPromo)   hlRows.push([' Top Promo',       highlights.bestPromo.id,                                              `${highlights.bestPromo.timesUsed} uses`])
      ...
      if (report.perBranch?.length) {
        ...
        drawTable(['Branch', 'Appointments', 'Gross', 'VAT', 'Final'],
          report.perBranch.map(r => [r.branchData?.name || r.id, r.count, peso(r.grossTotal), peso(r.vatAmount), peso(r.finalAmount)]))
      }
      ...
      if (report.promos?.length) {
        ...
        drawTable(['Code', 'Type', 'Value', 'Times Used', 'Total Discount', 'Total Final'],
          report.promos.map(r => [r.id, r.discountType, r.discountType === 'percent' ? r.discountValue + '%' : peso(r.discountValue), r.timesUsed, peso(r.totalDiscount), peso(r.totalFinal)]))
      }
  ...
  const handleExportCSV = () => {
    ...
    if (highlights?.bestBranch)  addSection('HIGHLIGHTS - BEST BRANCH',  ['name','final_amount'], [[highlights.bestBranch.branchData?.name || highlights.bestBranch.id, highlights.bestBranch.finalAmount]])
    ...
    if (highlights?.bestPromo)   addSection('HIGHLIGHTS - TOP PROMO',     ['code','times_used'],   [[highlights.bestPromo.id, highlights.bestPromo.timesUsed]])
    if (report.perBranch?.length) addSection('PER BRANCH', ['branch','appointments','gross','vat','final'], report.perBranch.map(r => [r.branchData?.name || r.id, r.count, r.grossTotal, r.vatAmount, r.finalAmount]))
    ...
    if (report.promos?.length) addSection('PROMO CODES', ['code','type','value','times_used','total_discount','total_final'], report.promos.map(r => [r.id, r.discountType, r.discountValue, r.timesUsed, r.totalDiscount, r.totalFinal]))
  ...
  const handleExportExcel = async () => {
    if (!report) return
    const BLUE = 'FF2563EB'; const BLIGHT = 'FFDBEAFE'; const WHITE = 'FFFFFFFF'; const DARK = 'FF1E1E32'; const GRAY = 'FF787890'
    ...
    const addSheet = (name, headers, rows) => {
      ...
      sc.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: BLIGHT } }
      ...
      rows.forEach((row, i) => {
        ...
        dr.eachCell({ includeEmpty: true }, cell => {
          ...
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: isAlt ? BLIGHT : WHITE } }
    ...
    if (highlights) addSheet('Highlights', ['Category','Name','Value'], [
      ...(highlights.bestBranch  ? [['Best Branch',    highlights.bestBranch.branchData?.name  || highlights.bestBranch.id,  peso(highlights.bestBranch.finalAmount)]]  : []),
      ...(highlights.worstBranch ? [['Needs Attention', highlights.worstBranch.branchData?.name || highlights.worstBranch.id, peso(highlights.worstBranch.finalAmount)]] : []),
      ...(highlights.bestService ? [['Top Service',    highlights.bestService.name,                                            peso(highlights.bestService.revenue)]]     : []),
      ...(highlights.bestPromo   ? [['Top Promo Code', highlights.bestPromo.id,                                               highlights.bestPromo.timesUsed + ' uses']] : []),
      ...(highlights.bestPeriod  ? [['Best Period',    highlights.bestPeriodLabel,                                             peso(highlights.bestPeriod.finalAmount)]]  : []),
    ])
    if (report.perBranch?.length) addSheet('Per Branch', ['Branch','Appointments','Gross','VAT','Final'], report.perBranch.map(r => [r.branchData?.name || r.id, r.count, peso(r.grossTotal), peso(r.vatAmount), peso(r.finalAmount)]))
    ...
    if (report.promos?.length) addSheet('Promo Codes', ['Code','Type','Value','Times Used','Total Discount','Total Final'], report.promos.map(r => [r.id, r.discountType, r.discountType === 'percent' ? r.discountValue + '%' : peso(r.discountValue), r.timesUsed, peso(r.totalDiscount), peso(r.totalFinal)]))
    if (report.summary?.length) addSheet('Period Summary', ['Period','Orders','Gross','Discount','VAT','Final'], report.summary.map(r => {
      const id = r.id; const lbl = id.h != null ? `${id.y}-${String(id.m).padStart(2,'0')}-${String(id.d).padStart(2,'0')} ${String(id.h).padStart(2,'0')}:00` : id.d != null ? `${id.y}-${String(id.m).padStart(2,'0')}-${String(id.d).padStart(2,'0')}` : `${id.y}-${String(id.m).padStart(2,'0')}`
      return [lbl, r.count, peso(r.grossTotal), peso(r.totalDiscount), peso(r.vatAmount), peso(r.finalAmount)]
    }))
  ...
  const inputClass = "px-4 py-2.5 border border-blue-100 font-sans text-sm text-neutral-700 placeholder-neutral-300 focus:outline-none focus:border-blue-400 transition-colors bg-white"
      ...
      <div className="px-10 pt-10 pb-12"
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            {report && <p className="font-sans text-sm text-blue-200 mt-2">{periodLabel()}</p>}
          </div>
          {report && (
              ...
              {FILTER_OPTIONS.map(f => (
                <button key={f} onClick={() => setPreset(f)}
                  className={`font-sans text-xs uppercase tracking-[0.2em] font-bold px-5 py-2.5 transition-colors capitalize ${
                    preset === f ? 'bg-blue-600 text-white' : 'bg-white text-neutral-400 hover:text-blue-600'
                  }`}>
          ...
          <div ref={reportRef} className="space-y-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  {highlights.bestBranch && (
                    <HighlightCard
                      ...
                      value={highlights.bestBranch.branchData?.name || highlights.bestBranch.id}
                  ...
                  {highlights.worstBranch && (
                    <HighlightCard
                      ...
                      value={highlights.worstBranch.branchData?.name || highlights.worstBranch.id}
            ...
            {isAdmin && report.perBranch?.length > 0 && (
              <div>
                ...
                <div className="bg-white border border-blue-100 divide-y divide-blue-50">
                  <div className="grid grid-cols-5 px-7 py-3 bg-blue-50">
                  ...
                  {report.perBranch.map((r, i) => (
                    <div key={i} className={`grid grid-cols-5 items-center px-7 py-4 hover:bg-blue-50 transition-colors ${highlights?.bestBranch?.id === r.id ? 'bg-amber-50/50' : ''}`}>
                      <p className="font-sans text-sm font-semibold text-neutral-800 flex items-center gap-2">
                        
                        {r.branchData?.name || r.id}
                      </p>
                      ...
                      <p className="font-sans text-sm font-black text-blue-900 text-right">{peso(r.finalAmount)}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {report.perService?.length > 0 && (
              <div>
                ...
                <Divider />
                <div className="bg-white border border-blue-100 divide-y divide-blue-50">
                  <div className="grid grid-cols-4 px-7 py-3 bg-blue-50">
                    {['Service', 'Times Booked', 'Unit Price', 'Revenue'].map((h, i) => (
                      <p key={h} className={`font-sans text-[10px] uppercase tracking-[0.3em] text-blue-400 font-bold ${i > 0 ? 'text-right' : ''}`}>{h}</p>
                    ))}
                  </div>
                  {report.perService.map((r, i) => (
                    <div key={i} className={`grid grid-cols-4 items-center px-7 py-4 hover:bg-blue-50 transition-colors ${highlights?.bestService?.name === r.name ? 'bg-blue-50/60' : ''}`}>
                      <p className="font-sans text-sm font-semibold text-neutral-800 flex items-center gap-2">
                        {r.name}
                      </p>
                      <p className="font-sans text-sm font-semibold text-neutral-500 text-right">{r.count}</p>
                      <p className="font-sans text-sm font-semibold text-neutral-500 text-right">{peso(r.price)}</p>
                      <p className="font-sans text-sm font-black text-blue-900 text-right">{peso(r.revenue)}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-white border border-blue-100 divide-y divide-blue-50">
                  ...
                  {report.promos.map((r, i) => (
                    <div key={i} className={`grid grid-cols-5 items-center px-7 py-4 hover:bg-blue-50 transition-colors ${highlights?.bestPromo?.id === r.id ? 'bg-green-50/40' : ''}`}>
                      <span className="font-sans text-xs font-black text-blue-700 uppercase tracking-[0.15em] bg-blue-50 border border-blue-100 px-2 py-0.5 inline-flex items-center gap-1.5 w-fit">
                        {highlights?.bestPromo?.id === r.id && <span className="text-green-500">🎯</span>}
                        {r.id}
                      </span>
                      ...
                      <p className="font-sans text-sm font-black text-blue-900 text-right">{peso(r.totalFinal)}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {report.summary?.length > 0 && (
              <div>
                ...
                <Divider />
                <div className="bg-white border border-blue-100 divide-y divide-blue-50">
                  <div className="grid grid-cols-6 px-7 py-3 bg-blue-50">
  ...
  const lbl = id.h != null
    ? `${id.y}-${String(id.m).padStart(2,'0')}-${String(id.d).padStart(2,'0')} ${String(id.h).padStart(2,'0')}:00`
    : id.d != null ? `${id.y}-${String(id.m).padStart(2,'0')}-${String(id.d).padStart(2,'0')}`
    : id.y != null && id.m != null ? `${id.y}-${String(id.m).padStart(2,'0')}`
    : '—'
                    const isBest = highlights?.bestPeriodLabel === lbl
                    return (
                      <div key={i} className={`grid grid-cols-6 items-center px-7 py-4 hover:bg-blue-50 transition-colors ${isBest ? 'bg-indigo-50/40' : ''}`}>
