...
const ROLE_META = {
  ...
  branchadmin: { label: 'Branch', Icon: Store,       border: 'border-purple-200 text-purple-600' },
...
const DetailModal = ({ log, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
      <div className="bg-white border border-violet-100 w-full max-w-lg max-h-[85vh] flex flex-col"
        ...
        <div className="px-6 py-4 border-b border-violet-100"
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-violet-200 hover:text-white font-sans text-lg leading-none">✕</button>
          </div>
          <p className="font-sans text-[10px] text-violet-300 uppercase tracking-[0.2em] mt-1">{formatDate(log.createdAt)}</p>
        </div>
        <div className="overflow-y-auto px-6 py-5 space-y-4 text-sm">
          ...
          <div>
            <p className="uppercase tracking-[0.2em] text-[10px] font-sans font-semibold text-violet-400 mb-1.5">Actor</p>
            <div className="flex items-center gap-2">
          ...
          {log.target?.label && (
            <div>
              <p className="uppercase tracking-[0.2em] text-[10px] font-sans font-semibold text-violet-400 mb-1.5">Target</p>
          ...
          {(log.before || log.after) && (
            <div className="grid grid-cols-2 gap-3">
              {log.before && (
                <div>
                  <p className="uppercase tracking-[0.2em] text-[10px] font-sans font-semibold text-violet-400 mb-1.5">Before</p>
              ...
              {log.after && (
                <div>
                  <p className="uppercase tracking-[0.2em] text-[10px] font-sans font-semibold text-violet-400 mb-1.5">After</p>
          ...
          {log.meta && Object.keys(log.meta).length > 0 && (
            <div>
              <p className="uppercase tracking-[0.2em] text-[10px] font-sans font-semibold text-violet-400 mb-1.5">Meta</p>
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif" }} className="min-h-screen bg-white">
      ...
      <div className="px-10 pt-10 pb-12"
        style={{ background: 'radial-gradient(ellipse at top right, rgba(255,255,255,0.12) 0%, transparent 60%), #7c3aed' }}>
        <p className="uppercase tracking-[0.35em] text-[10px] text-violet-200 font-sans mb-3">Settings</p>
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="font-sans text-sm text-violet-300 mt-2">
      ...
      <div className="px-10 py-10 max-w-7xl mx-auto">
        ...
        {/* ── Section label ── */}
        <p className="uppercase tracking-[0.35em] text-[10px] text-violet-400 font-sans mb-2">
          {isLoginHistory ? 'Filter Login Events' : 'Filter Audit Logs'}
        </p>
        <div className="h-px bg-violet-100 mb-6" />

        {/* ── Filters ── */}
        <div className="bg-white border border-violet-100 mb-8">
          <div className="bg-violet-50 px-7 py-3 border-b border-violet-100">
            <span className="uppercase tracking-[0.2em] text-[10px] font-sans font-semibold text-violet-400">Filters</span>
          </div>
          <div className="px-7 py-4 flex flex-wrap gap-3 items-end">
            ...
            <div className="flex flex-col gap-1">
              <span className="uppercase tracking-[0.2em] text-[9px] font-sans font-semibold text-violet-400">
            ...
            {!isLoginHistory && (
              <div className="flex flex-col gap-1">
                <span className="uppercase tracking-[0.2em] text-[9px] font-sans font-semibold text-violet-400">Role</span>
            ...
            {!isLoginHistory && (
              <div className="flex flex-col gap-1">
                <span className="uppercase tracking-[0.2em] text-[9px] font-sans font-semibold text-violet-400">Target</span>
            ...
            {/* Date From */}
            <div className="flex flex-col gap-1">
              <span className="uppercase tracking-[0.2em] text-[9px] font-sans font-semibold text-violet-400">Date From</span>
            ...
            {/* Date To */}
            <div className="flex flex-col gap-1">
              <span className="uppercase tracking-[0.2em] text-[9px] font-sans font-semibold text-violet-400">Date To</span>
            ...
            {hasActiveFilters && (
              <button onClick={clearFilters}
                className="font-sans text-xs text-violet-400 hover:text-violet-700 underline transition-colors self-end pb-2.5">
                Clear filters
              </button>
            )}
          </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="font-sans text-xs text-neutral-400">
            Showing <span className="text-violet-600 font-semibold">{total}</span> {total === 1 ? 'entry' : 'entries'}
          </p>
        </div>
        ...
        <div className="bg-white border border-violet-100 overflow-hidden">
          ...
          {/* Header row */}
          <div className={`grid bg-violet-50 px-7 py-3 border-b border-violet-100 ${
            isLoginHistory
              ...
              {logs.map(log => {
                ...
                return (
                  <div key={log._id} onClick={() => setSelectedLog(log)}
                    className={`grid items-center px-7 py-4 hover:bg-violet-50 transition-colors cursor-pointer ${
                      isLoginHistory
                        ? 'grid-cols-[2fr_0.7fr_0.7fr_1.2fr]'
                        : 'grid-cols-[2fr_1fr_0.8fr_1fr_1.2fr]'
                    }`}>
                  ...
                  return (
                    <button key={p} onClick={() => setFilter('page', p)}
                      className={`w-7 h-7 font-sans text-xs font-bold transition-colors ${
                        p === filters.page ? 'bg-violet-600 text-white' : 'text-violet-400 hover:text-violet-700'
                      }`}
