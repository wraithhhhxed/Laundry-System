...
// Normalize a slotDate (YYYY-MM-DD or ISO) into YYYY-MM-DD for comparison
...
const AllAppointments = () => {
  ...
  const [dateFilter,     setDateFilter]     = useState('')   // ← NEW: YYYY-MM-DD
  ...
  useEffect(() => { setCurrentPage(1) }, [search, selectedBranch, statusFilter, paymentFilter, sourceFilter, dateFilter])
  ...
  const filtered = useMemo(() => {
    ...
    return appointments.filter(appt => {
      ...
      // ── NEW: Date filter (matches appt.slotDate) ──
      if (dateFilter) {
        if (toDateKey(appt.slotDate) !== dateFilter) return false
      }
  ...
  }, [appointments, selectedBranch, search, statusFilter, paymentFilter, sourceFilter, dateFilter])
  ...
  const hasFilters   = search || selectedBranch !== 'all' || statusFilter !== 'all' || paymentFilter !== 'all' || sourceFilter !== 'all' || dateFilter
  const clearFilters = () => { setSearch(''); setSelectedBranch('all'); setStatusFilter('all'); setPaymentFilter('all'); setSourceFilter('all'); setDateFilter('') }
  ...
  // Human-friendly label for the active date filter
  const dateFilterLabel = dateFilter
    ? new Date(dateFilter + 'T00:00:00').toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' })
    : null
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif" }} className="min-h-screen bg-white">
      ...
      <div className="px-10 py-10 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap mb-3">
          ...
          <div className="relative flex items-center gap-2">
            <input
              ...
              className="px-4 py-2.5 border border-blue-100 font-sans text-sm text-neutral-700 focus:outline-none focus:border-blue-400 transition-colors bg-white cursor-pointer"
              aria-label="Filter by appointment date"
            />
            {dateFilter && (
              <button
                onClick={() => setDateFilter('')}
                className="font-sans text-xs uppercase tracking-[0.2em] text-blue-400 hover:text-blue-600 transition-colors whitespace-nowrap"
        ...
        {dateFilterLabel && (
          <div className="flex items-center gap-2 mb-4 bg-blue-50 border border-blue-200 px-4 py-2 w-fit">
            <span className="font-sans text-xs text-blue-600">
              Showing appointments for <span className="font-bold">{dateFilterLabel}</span>
            </span>
            <button
              onClick={() => setDateFilter('')}
              className="font-sans text-xs text-blue-400 hover:text-blue-700 font-bold leading-none"
              title="Clear date filter"
            >
              ×
            </button>
