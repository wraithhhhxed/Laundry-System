      ...
      <div className='px-7 pb-10'>
        ...
        <div className='bg-white border border-blue-100 overflow-hidden'>
          {loading ? (
            ...
            <div>
              <div className='grid grid-cols-[2fr_1fr_2fr_1fr_1fr_auto] bg-blue-50 px-7 py-3 border-b border-blue-100'>
              ...
              <div className='divide-y divide-blue-50'>
                {branches.map((branch) => {
                  ...
                  return (
                    <div key={branch.id} className='grid grid-cols-[2fr_1fr_2fr_1fr_1fr_auto] items-center px-7 py-4 hover:bg-blue-50 transition-colors'>
                      ...
                      <div className='flex flex-wrap gap-1'>
                        {shortNames.slice(0, 2).map(s => (
                          <span key={s} className='uppercase tracking-[0.2em] text-[10px] font-sans font-bold border border-blue-200 text-blue-500 px-2 py-0.5' title={branch.speciality?.[shortNames.indexOf(s)] || s}>{s}</span>
                        ))}
                        {shortNames.length > 2 && <span className='font-sans text-xs text-neutral-400'>+{shortNames.length - 2}</span>}
                      </div>
                      <div className='font-sans font-black text-sm text-blue-700'>₱{branch.fees?.toLocaleString() || '—'}</div>
                      <div>
                        {branch.available ? (
